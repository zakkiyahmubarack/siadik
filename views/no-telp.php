			...
			?>
					...
					<table class="table table-striped table-bordered">
						...
						if (url_segment(2) == "search") {
							...
							$h2 = mysqli_query($koneksi, "select * from tbl_phone a LEFT JOIN tbl_user b on a.kode_user=b.kode_user LEFT JOIN tbl_phone_category c on a.id_phone_category=c.id order by a.id_phone DESC LIMIT $posisi,$batas");
						}
				...
				?>
					<form name="form1" method="post" action="<?php echo base_url() . "/" . url_segment(1) . "/simpan"; ?>" class="form">
						...
						<p>Kategori Nomor HP:<br>
							<select class="form-control" name="id_phone_category" id="user" required="true">
								<option value="">.::Pilih Data:.</option>

								<?php
								$h3 = mysqli_query($koneksi, "select * from tbl_phone_category");
								while ($d3 = mysqli_fetch_array($h3)) {
								?>
				...
				case 'simpan':
					if (empty($_POST['no'])) {
						...
						$h3 = mysqli_query($koneksi, "insert into tbl_phone (no_phone,kode_user,id_phone_category) 
											values ('$_POST[no]','$_POST[user]','$_POST[id_phone_category]')");
						if ($h3 == TRUE) {
					...
					break;
				case 'edit':
					$data = mysqli_fetch_assoc(mysqli_query($koneksi, "select * from tbl_phone a LEFT JOIN tbl_user b on a.kode_user=b.kode_user LEFT JOIN tbl_phone_category c on a.id_phone_category=c.id where a.id_phone='" . url_segment(3) . "'"));
				?>
					<form name="form1" method="post" action="<?php echo base_url() . "/" . url_segment(1) . "/update"; ?>" class="form">
						...
						<p>Kategori Nomor HP:<br>
							<select class="form-control" name="id_phone_category" id="user" required="true">
								<option value="<?php echo $data['id_phone_category']; ?>"><?php echo $data['nama']; ?></option>


								<?php
								$h3 = mysqli_query($koneksi, "select * from tbl_phone_category");
								while ($d3 = mysqli_fetch_array($h3)) {
									if ($d3['id'] <> $data['id_phone_category']) {
				...
				case 'update':
					if (empty($_POST['no'])) {
						...
						$h3 = mysqli_query($koneksi, "UPDATE tbl_phone set
											...
											id_phone_category='$_POST[id_phone_category]'
